...
def save_for_sm_hf_inference(model, model_config, model_dir, seq_length=1024, translate_to_hf=False):
    if smp.rdp_rank() == 0:
        model_state_dict = model.state_dict(gather_to_rank0=True)
        if smp.rank() == 0:
            torch.save(translate_state_dict_to_hf_gptj(model_state_dict, seq_length) if translate_to_hf else model_state_dict,
                        os.path.join(model_dir, 'gptj.pt'))
            ...
            dst_inference_script = os.path.join(model_dir, "code")
...
def main():
    ...
    if is_sagemaker_mp_enabled():
        ...
        if training_args.save_final_full_model:
            ...
            save_for_sm_hf_inference(model, AutoConfig.from_pretrained(model_args.model_name_or_path), training_args.model_dir, seq_length=1024, translate_to_hf=smp.tp_size() > 1)
